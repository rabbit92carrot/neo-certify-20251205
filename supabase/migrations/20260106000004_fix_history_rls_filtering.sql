-- =============================================================================
-- get_history_summary_cursor RLS 필터링 수정
--
-- 문제: 기존 WHERE 절이 from_owner_id OR to_owner_id로 너무 관대함
--       - 유통사가 병원에게 출고 시 SHIPPED + RECEIVED 모두 유통사에게 표시됨
--       - RECEIVED는 수신자(병원)만 봐야 하는데 발송자(유통사)도 보게 됨
--
-- 수정: 이벤트 타입별로 적절한 필터링 적용
--       - SHIPPED: 내가 발송한 건만 (from_owner_id = me)
--       - RECEIVED: 내가 수신한 건만 (to_owner_id = me)
--       - RETURNED/RECALLED: 내가 발송하거나 수신한 건 모두 (from OR to)
--       - TREATED/PRODUCED/DISPOSED: 내가 수행한 건만 (from_owner_id = me)
-- =============================================================================

-- 기존 함수 삭제 (반환 타입 동일하므로 CREATE OR REPLACE로 충분하지만 명시적으로)
...
-- 함수 설명
COMMENT ON FUNCTION public.get_history_summary_cursor IS '거래 이력 요약 조회 (커서 기반 페이지네이션). 이벤트 타입별 RLS 필터링 적용.';
