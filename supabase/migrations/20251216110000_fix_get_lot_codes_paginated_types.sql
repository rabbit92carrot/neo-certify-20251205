-- =====================================================
-- Migration: 20251216110000_fix_get_lot_codes_paginated_types
-- Description: get_lot_codes_paginated 함수 반환 타입 수정
-- Fix: owner_type이 enum이므로 TEXT로 캐스팅하여 비교
-- =====================================================

-- 기존 함수 삭제 후 재생성 (타입 수정)
DROP FUNCTION IF EXISTS get_lot_codes_paginated(UUID, INTEGER, INTEGER);

-- Lot별 고유식별코드 페이지네이션 조회 함수 (타입 수정 버전)
...
BEGIN
  ...
  -- 페이지네이션된 결과 반환
  -- owner_type을 TEXT로 캐스팅하여 enum 타입 문제 해결
  RETURN QUERY
  SELECT
    vc.id,
    vc.code,
