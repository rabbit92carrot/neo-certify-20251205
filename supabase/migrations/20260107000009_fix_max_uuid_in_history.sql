...
--
-- 문제:
-- PostgreSQL의 MAX() 함수는 UUID 타입을 지원하지 않음
-- "function max(uuid) does not exist" 오류 발생
--
-- 해결:
-- MAX(shipment_batch_id)를 (ARRAY_AGG(...))[1]로 변경
-- 동일 그룹 내의 값은 모두 같으므로 첫 번째 값 선택
-- =============================================================================
...
BEGIN
  ...
  WITH grouped_histories AS (
    SELECT
      ...
      -- 수정: MAX(uuid) 대신 ARRAY_AGG 사용 (그룹 내 동일 값)
...
COMMENT ON FUNCTION public.get_history_summary_cursor(uuid, text[], timestamptz, timestamptz, boolean, integer, timestamptz, text) IS
  '조직의 이력 요약을 커서 기반 페이지네이션으로 조회.
   v3: MAX(uuid) 오류 수정 - ARRAY_AGG로 변경.
