-- Fix: get_admin_event_summary_cursor 함수의 grp_key에 action_type 포함
--
-- 문제: grp_key가 batch ID만 사용할 경우 action_type이 누락되어
--       동일 배치의 SHIPPED/RECEIVED 이벤트가 같은 grp_key를 가짐
--       → React에서 중복 key 오류 발생
--
-- 해결: grp_key에 항상 action_type을 포함하도록 수정
...
BEGIN
  ...
  grouped AS (
    SELECT
      -- FIX: grp_key에 항상 action_type 포함
      COALESCE(
    ...
    GROUP BY
      -- FIX: GROUP BY에도 동일하게 action_type 포함
      COALESCE(
        fh.shipment_batch_id::TEXT,
        fh.history_lot_id::TEXT,
        fh.treatment_id::TEXT,
