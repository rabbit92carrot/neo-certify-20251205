-- =============================================================================
-- grp_key에 action_type 포함하여 중복 이벤트 문제 해결
--
-- 문제:
-- grp_key가 shipment_batch_id만 사용하지만, GROUP BY에서 action_type으로 분리
-- 결과: 같은 grp_key를 가진 SHIPPED/RECEIVED 2개 행이 반환됨
--
-- 해결:
-- grp_key 생성 시 action_type도 포함하여 완전한 분리 보장
-- - SHIPPED: grp_key = 'abc123_SHIPPED'
-- - RECEIVED: grp_key = 'abc123_RECEIVED'
-- =============================================================================
...
BEGIN
  ...
  WITH grouped_histories AS (
    SELECT
      -- 수정: grp_key에 action_type 포함하여 완전한 분리 보장
      -- 주의: action_type은 enum 타입이므로 TEXT로 명시적 캐스팅 필요
      COALESCE(
        h.shipment_batch_id::TEXT || '_' || h.action_type::TEXT,  -- 배치 + 액션 타입
        h.lot_id::TEXT || '_' || h.action_type::TEXT,             -- Lot + 액션 타입
        h.treatment_id::TEXT || '_' || h.action_type::TEXT,       -- 시술 + 액션 타입
      ...
      -- MAX(uuid) 대신 ARRAY_AGG 사용 (그룹 내 동일 값)
    ...
    GROUP BY
      -- 수정: grp_key에 action_type 포함 (TEXT 캐스팅)
      COALESCE(
...
COMMENT ON FUNCTION public.get_history_summary_cursor(uuid, text[], timestamptz, timestamptz, boolean, integer, timestamptz, text) IS
  '조직의 이력 요약을 커서 기반 페이지네이션으로 조회.
   v4: grp_key에 action_type 포함하여 중복 이벤트 문제 해결.
   owned_quantity 포함 - 반품 버튼 표시 여부 결정에 사용.';
