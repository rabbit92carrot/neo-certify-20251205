-- 관리자 이벤트 요약 커서 함수를 SSOT 기반으로 업데이트
--
-- 변경 사항:
...
-- 2. lot_summaries에 modelName 추가
-- 3. batch_id 반환 (디버깅/추적용)

-- 반환 타입이 변경되었으므로 기존 함수를 먼저 삭제
DROP FUNCTION IF EXISTS public.get_admin_event_summary_cursor(
  timestamptz, timestamptz, text[], uuid, text, uuid, boolean, integer, timestamptz, text
);
