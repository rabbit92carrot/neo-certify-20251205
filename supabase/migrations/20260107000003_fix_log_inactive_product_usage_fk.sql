-- =============================================================================
-- log_inactive_product_usage FK 에러 방지 수정
--
-- 문제:
-- ADMIN 조직이 삭제된 상태에서 함수 호출 시 FK 제약 조건 위반 발생
-- (recipient_org_id가 organizations 테이블에 없는 경우)
--
-- 해결:
-- FK 에러 발생 시 해당 조직 건너뛰기 (EXCEPTION WHEN foreign_key_violation)
-- =============================================================================
...
-- 함수 소유자 설정
ALTER FUNCTION "public"."log_inactive_product_usage"(
  character varying, uuid, uuid, uuid, integer
) OWNER TO postgres;

-- 함수 설명
COMMENT ON FUNCTION "public"."log_inactive_product_usage"(
  character varying, uuid, uuid, uuid, integer
) IS '비활성 제품 사용 로그 기록 및 알림 생성. FK 에러 방어 로직 포함 (삭제된 조직 건너뛰기).';
