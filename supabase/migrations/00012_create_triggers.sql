...
COMMENT ON TRIGGER trg_lot_create_virtual_codes ON lots IS
  'Lot 생성 시 수량만큼 가상 식별코드 자동 생성';
...
COMMENT ON TRIGGER trg_virtual_code_produced ON virtual_codes IS
  '가상 식별코드 생성(생산) 시 PRODUCED 이력 자동 기록';
...
COMMENT ON TRIGGER trg_organization_manufacturer_settings ON organizations IS
  '제조사 조직 생성 시 기본 설정 자동 생성';
...
COMMENT ON TRIGGER trg_treatment_ensure_patient ON treatment_records IS
  '시술 기록 생성 시 환자 레코드 자동 생성 (없는 경우)';
...
COMMENT ON TRIGGER trg_notification_ensure_patient ON notification_messages IS
  '알림 메시지 생성 시 환자 레코드 자동 생성 (없는 경우)';
