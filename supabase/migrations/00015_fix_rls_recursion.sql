-- Migration: 00015_fix_rls_recursion
...
COMMENT ON FUNCTION user_owns_codes_in_lot(UUID) IS '사용자가 해당 Lot의 가상 코드를 소유하는지 확인 (RLS 재귀 방지)';
...
COMMENT ON FUNCTION user_is_lot_manufacturer(UUID) IS '사용자가 해당 Lot의 제품 제조사인지 확인 (RLS 재귀 방지)';
