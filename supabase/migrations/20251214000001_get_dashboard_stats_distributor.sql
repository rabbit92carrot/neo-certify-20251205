...
CREATE OR REPLACE FUNCTION get_dashboard_stats_distributor(p_organization_id UUID)
RETURNS TABLE (
...
-- 권한 부여
GRANT EXECUTE ON FUNCTION get_dashboard_stats_distributor(UUID) TO authenticated;

COMMENT ON FUNCTION get_dashboard_stats_distributor(UUID) IS
  '유통사 대시보드 통계 통합 조회. 4개 쿼리를 1개 RPC로 통합하여 DB 왕복 감소';
