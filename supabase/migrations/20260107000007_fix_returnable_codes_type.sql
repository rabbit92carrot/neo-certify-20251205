-- =============================================================================
-- 반품 가능 코드 조회 함수 타입 수정
--
-- 문제:
-- ARRAY_AGG(cd.code_string)이 VARCHAR[] 반환하지만, 함수는 TEXT[] 선언
-- PostgreSQL 오류: "Returned type character varying[] does not match expected type text[]"
--
-- 해결:
-- ARRAY_AGG에 ::TEXT 캐스팅 추가
-- =============================================================================

-- 기존 함수 삭제
DROP FUNCTION IF EXISTS "public"."get_returnable_codes_by_batch"(UUID);

-- 수정된 함수 생성 (TEXT[] 캐스팅 적용)
...
BEGIN
  ...
  SELECT
    ...
    -- FIX: VARCHAR[]를 TEXT[]로 캐스팅
    ARRAY_AGG(cd.code_string::TEXT) FILTER (WHERE cd.is_owned = 1) AS codes
...
COMMENT ON FUNCTION "public"."get_returnable_codes_by_batch"(UUID) IS
  '반품 가능 코드 조회. 배치의 제품별 원래 수량과 현재 보유 수량을 반환.
   ...
   v2: VARCHAR[]를 TEXT[]로 캐스팅하여 타입 불일치 해결.';
