...
--
-- 문제:
-- 부분 반품 시 빈 배열 '[]'이 전달되면 jsonb_to_recordset 에러 발생
-- (UI에서 수량 0인 제품만 선택 시)
--
-- 해결:
-- 빈 배열은 NULL과 동일하게 처리 (전량 반품)
-- =============================================================================

-- 기존 함수 삭제 (시그니처 동일)
DROP FUNCTION IF EXISTS "public"."return_shipment_atomic"(UUID, VARCHAR, JSONB);

-- 수정된 함수 생성 (빈 배열 방어 추가)
...
BEGIN
  ...
  -- 2. 빈 배열 정규화: 빈 배열 '[]'은 NULL로 처리 (전량 반품)
  IF p_product_quantities IS NOT NULL AND jsonb_array_length(p_product_quantities) = 0 THEN
