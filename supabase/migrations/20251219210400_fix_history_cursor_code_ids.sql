-- Fix get_history_summary_cursor function to return actual virtual codes instead of UUIDs
-- Problem: code_ids was collecting h.virtual_code_id (UUID) instead of vc.code (NC-XXXXXXXX format)
-- This affects all organization history pages (manufacturer, distributor, hospital)
...
BEGIN
  ...
  WITH grouped_histories AS (
    SELECT
      ...
      p.id AS prod_id,
      p.name AS prod_name,
      -- FIX: Use vc.code (NC-XXXXXXXX format) instead of h.virtual_code_id (UUID)
...
COMMENT ON FUNCTION public.get_history_summary_cursor IS '조직별 이력 요약 커서 기반 페이지네이션 - fixed to return actual virtual codes (NC-XXXXXXXX) instead of UUIDs';
