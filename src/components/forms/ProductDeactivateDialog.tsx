...
interface ProductDeactivateDialogProps {
  ...
  onConfirm: (productId: string, reason: ProductDeactivationReason, note?: string) => Promise<void>;
}
...
export function ProductDeactivateDialog({
  ...
  const [reason, setReason] = useState<ProductDeactivationReason | ''>('');
  const [note, setNote] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  // 사유별 아이콘
  const getReasonIcon = (reasonType: ProductDeactivationReason) => {
    switch (reasonType) {
      case 'SAFETY_ISSUE':
  ...
  // 사유별 설명
  const getReasonDescription = (reasonType: ProductDeactivationReason) => {
    switch (reasonType) {
      case 'SAFETY_ISSUE':
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onConfirm(product.id, reason, note || undefined);
      // 초기화
      setReason('');
      setNote('');
      onOpenChange(false);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            <span className="font-semibold">{product?.name}</span> 제품을 비활성화합니다.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={reason}
              onValueChange={(value) => setReason(value as ProductDeactivationReason)}
            >
              <SelectTrigger id="reason">
                <SelectValue placeholder="사유를 선택하세요" />
              </SelectTrigger>
              <SelectContent>
                {(Object.entries(DEACTIVATION_REASON_LABELS) as [ProductDeactivationReason, string][]).map(
                  ([key, label]) => (
